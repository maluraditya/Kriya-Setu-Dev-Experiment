...
const BufferSolutionsLab: React.FC<Props> = ({ topic, onExit }) => {
    ...
    const statusBadge = (
        <div className="flex flex-col items-center px-4 py-2 bg-slate-800/80 rounded-2xl backdrop-blur-md border border-white/10 shadow-xl max-w-xl text-center">
            <div className="text-[12px] text-white/70 font-medium leading-tight">
                Observe how buffer solutions resist changes in pH when small amounts of acid or base are added compared to pure water.
            </div>
        </div>
    );
    ...
    const controlsCombo = (
        <div className="w-full flex flex-wrap lg:flex-nowrap gap-4">
            <div className="flex-[2] bg-slate-950/50 p-3 flex flex-col gap-2 rounded-xl border border-slate-700/50 min-w-[200px]">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Buffer System</label>
                <select value={bufferSystem}
                    onChange={e => { setBufferSystem(e.target.value as BufferSystem); setBufHCl(0); setBufNaOH(0); }}
                    className="w-full bg-slate-800 border border-slate-600 text-xs text-white rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500 cursor-pointer h-full">
                    <option value="acetic">Acetic Acid / Acetate (pKₐ 4.76)</option>
                    <option value="ammonia">Ammonia / Ammonium (pKᵦ 4.75)</option>
                </select>
            </div>

            <div className="flex-[2] bg-slate-950/50 p-3 rounded-xl border border-slate-700/50 min-w-[150px]">
            ...
            <div className="flex-1 bg-slate-950/50 p-3 flex flex-col gap-2 rounded-xl border border-slate-700/50 min-w-[100px]">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block invisible">Equation</label>
                <button onClick={() => setShowMath(!showMath)}
                    className={`h-full w-full py-2 px-3 rounded-lg text-xs font-bold border transition-all cursor-pointer ${showMath ? 'bg-purple-500/20 border-purple-500/40 text-purple-300' : 'bg-slate-800 border-slate-600 text-slate-300 hover:bg-slate-700'}`}>
            ...
            <div className="flex-1 bg-slate-950/50 p-3 flex flex-col gap-2 rounded-xl border border-slate-700/50 min-w-[100px]">
                ...
                <button onClick={handleReset}
                    className="h-full w-full py-2 px-3 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-lg text-xs uppercase font-bold tracking-widest text-slate-300 transition-colors cursor-pointer flex items-center justify-center gap-2">
    ...
    const simulationCombo = (
        <div className="w-full h-full flex flex-col lg:flex-row items-center lg:items-center justify-center p-4 lg:p-8 gap-6 lg:gap-10 relative bg-transparent overflow-y-auto lg:overflow-hidden rounded-3xl min-h-0">
            {/* Grid background */}
            ...
            <div className="flex flex-col items-center justify-center gap-3 w-64 shrink-0 z-10">
                {/* Comparison arrow */}
                <div className="text-center mb-1">
                    <div className="text-sm font-bold text-white/50 bg-black/30 px-4 py-1.5 rounded-full border border-white/5">← Compare →</div>
                    <div className="text-[11px] text-white/40 mt-2 max-w-[200px] leading-relaxed mx-auto">
                ...
                {showMath && (
                    <div className="bg-slate-900/80 backdrop-blur-md border border-purple-500/30 shadow-[0_4px_20px_rgba(168,85,247,0.15)] rounded-2xl p-4 text-center w-full animate-in fade-in zoom-in duration-300">
                        <div className="text-[10px] text-purple-400 font-bold uppercase tracking-widest mb-2">Henderson-Hasselbalch</div>
                        <div className="font-mono text-xs sm:text-sm text-purple-200 leading-relaxed bg-black/40 p-2 rounded-xl">
                            {bufferSystem === 'acetic' ? (
                                <>
                            ...
                            <div className="mt-2 text-lg sm:text-xl font-bold bg-white/5 border border-white/10 rounded-lg py-1 px-3 inline-block" style={{ color: phColor(buf.pH) }}>
                                = {buf.pH.toFixed(2)}
                            </div>
                        </div>
                    </div>
                ...
                {/* Buffer ratio bar */}
                <div className="w-full bg-slate-900/80 border border-white/10 shadow-lg rounded-xl p-3 backdrop-blur-sm">
                    <div className="text-[9px] font-bold text-white/50 text-center mb-1.5 uppercase tracking-widest">Buffer Component Ratio</div>
                    <div className="flex h-3.5 rounded-md overflow-hidden gap-px">
                    ...
                    <div className="flex justify-between text-[10px] font-bold font-mono mt-1.5 px-0.5">
                        <span className="text-yellow-400/80">{acidLabel}</span>
                        <span className="text-blue-400/80">{saltLabel}</span>
                    </div>
                </div>

                {/* pH delta comparison */}
                <div className="w-full bg-slate-900/80 border border-white/10 shadow-lg rounded-xl p-3 text-center backdrop-blur-sm">
                    <div className="text-[9px] font-bold text-white/50 uppercase tracking-widest mb-2">pH Change from Neutral</div>
                    <div className="flex gap-4 justify-center items-center">
                        <div className="flex-1 bg-black/20 rounded-lg py-1.5">
                            <div className="text-[10px] font-medium text-white/40 mb-0.5">Water</div>
                        ...
                        <div className="flex-1 bg-black/20 rounded-lg py-1.5">
                            <div className="text-[10px] font-medium text-white/40 mb-0.5">Buffer</div>
